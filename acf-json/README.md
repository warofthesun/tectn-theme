...
- **Sitewide** (`tectn-sitewide`): group label at the **bottom** of the Site Settings submenu for Brand and Integrations. Sync `ui_options_page_tectn_sitewide.json`. Brand and Integrations are ordered below Sitewide with indented labels (`— Brand`, `— Integrations`) via `tectn_sort_site_settings_submenu()` in `inc/acf-options.php`.
- **Brand** (`site-settings`): submenu under **Sitewide** — logos only — `group_69a4e5ae0b03d.json`. Same `post_id` as before (`site-settings`) so `get_field( 'site_settings', 'site-settings' )` is unchanged.
- **Integrations** (`site-settings-integrations`): submenu under **Sitewide** — Google Maps API key — registered in `inc/acf-options.php` (`tectn_register_acf_integrations_field_group`). Read order: `tectn_google_maps_api_key()` in `inc/theme-setup.php`.
...
- **Resources** (`tectn-resources`): repeater of named sections (optional minimal headline, stable section ID, items with link + optional body text) — `group_tectn_site_resources.json`. Use `tectn_get_embedded_resource_sections()` or `tectn_find_resource_section_by_selector()` in PHP.
...
- **Block: Resources** (`tectn/resources`): `blocks/resources/` + `group_tectn_block_resources.json` — compose **sections** via flexible content: pick Site Settings sections by **stable section ID** (`resource_section_key`) with optional **page-only items**, add **page-only sections** inline, or add a **Site pages section** (manual page relationship, or auto-populate from **Page Categories** / **Page Tags**). Per-section **Hide header** and **Edit header** toggles (override preheader/headline on this page; turn off Edit header to restore originals). Block-level layout matches Information lists (background, columns, fill order, max width). Saving **Site Settings → Resources** backfills empty section IDs.
